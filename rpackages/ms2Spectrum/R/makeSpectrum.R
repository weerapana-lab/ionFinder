...
makeSpectrum <- function(specDat, simpleSequence = FALSE, includeMZLab = TRUE, nDigits = 3)
{
	...
	dat$ionType <- factor(x = dat$ionType, levels = c('blank', 'b', 'y', 'y_nl', 'b_nl', 'M_nl'))
	...
	mspect <- ggplot2::ggplot(data = dat, aes(x = mz, ymax = intensity, y = intensity, ymin = 0, color = ionType)) +
		geom_linerange()
	if(includeMZLab) {
	...
	dat.label <- subset(dat, dat$ionType %in% c("b", 'y', 'y_nl', 'b_nl', 'M_nl'))
	if(nrow(dat.label) > 0){
		mspect <- mspect + geom_text(data = dat.label,
									 aes(label = formatedLabel, x = labelX, y = labelY + BY_LAB_OFFSET_Y),
									 parse = TRUE,
									 size = 3)
	}
	mspect <- mspect + scale_y_continuous(breaks = y_labs, expand = c(0, 0), limits = c(0, Y_SCALE_LIM)) +
	scale_color_manual(values = c("b" = B_COLOR, "blank" = BLANK_COLOR, "y" = Y_COLOR, "y_nl" = Y_NL_COLOR, "b_nl" = B_NL_COLOR, 'M_nl' = M_NL_COLOR)) +
	theme_classic() +
